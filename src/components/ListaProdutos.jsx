...
export default function ListaProdutos(){
     ...
     useEffect(()=>{
          fetch("http://localhost:8080/LojaAulaSabado/rest/candidato").then((resp)=>{
               return resp.json();
          }).then((resp)=>{
               setProdutos(resp)
               console.log(resp)
     ...
     const handleDelete = (id)=>{
          fetch(`http://localhost:8080/LojaAulaSabado/rest/candidato/${id}`,{
               method:"delete"
          }).then(()=>{
               window.location = "/"
          }).catch((error)=>{
                         ...
                         {produtos.map((produto)=>(
                              <tr key={produto.codigo}>
                                   ...
                                   <td>
                                        ...
                                        <button onClick={handleDelete.bind(this, produto.codigo)}><FaTrash/></button> [//afirmando quem quero deletar]
                                   </td>
                              </tr>
                         ))}
                    </tbody>
                    <tfoot><tr colSpan='4'><td>Produtos do Servidor</td></tr></tfoot>
               </table>
          </DivLista>
     )
}